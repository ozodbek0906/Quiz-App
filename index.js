...
function parseQuestions(raw) {
  ...
  for (let block of parts) {
    block = block.trim();
    if (!block) continue;
    // First line contains number and question, e.g. "1. Text{"
    const lines = block.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
    if (lines.length === 0) continue;
    // Find the question line (first line that looks like starting with number.)
    let qlineIdx = 0;
    while (qlineIdx < lines.length && !/^\d+\./.test(lines[qlineIdx])) qlineIdx++;
    if (qlineIdx >= lines.length) qlineIdx = 0; // fallback
    let qline = lines[qlineIdx];
    // Remove number and trailing brace
    qline = qline.replace(/^\d+\./, '').replace(/\{\s*$/, '').trim();

    // Extract choices lines that start with ==== 
    const choiceLines = lines.filter(l => /^====/.test(l));
    const choices = [];
    let correctIndex = -1;
    choiceLines.forEach((cl, idx) => {
      let text = cl.replace(/^====/, '').trim();
      if (text.startsWith('#')) {
        correctIndex = idx;
        text = text.replace(/^#/, '').trim();
      } else if (text.includes('#')) {
        // if # appears somewhere inside
        const ci = text.indexOf('#');
        text = (text.slice(0, ci) + text.slice(ci+1)).trim();
        correctIndex = idx;
      }
      // normalize trailing periods/spaces
      choices.push(text);
    });
    if (choices.length === 0) continue;
    // If not found a correct index, leave as -1
